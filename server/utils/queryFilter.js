...
module.exports = async function queryFilter(queries) {
  ...
  const albumsMap = reducer(albums);
  const usersMap = reducer(users);
  ...
  if (queryEmail || queryAlbum) {
    ...
    for (let i = 0; i < albums.length; i++) {
      ...
      if (
        ...
        index[album.id] = {
          user: userIndex ?? usersMap[album.userId],
          album: i,
        };
      }
    }
      ...
      for (const photo of photos) {
        if (photo.albumId in index) {
          ...
          results.push(
            formatPhotoData(
              photo,
              albums[index[albumId].album],
              users[index[albumId].user],
            ),
          );
        }
      }
    ...
    for (const photo of photos) {
      ...
      const albumIdIndex = albumsMap[photo.albumId];
      const userIdIndex = usersMap[albums[albumIdIndex].userId];
